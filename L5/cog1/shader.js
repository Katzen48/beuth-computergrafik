...
define(["exports"], function(exports) {
	...
	function phongInterpolationPrepareScanline(interpolationVertexIndices, interpolationWeights, deltaX) {
		...
		for (var i = 0; i < 2; i++) {
			...
			vec3.subtract(
				vertexNormals[
					interpolationVertexIndices[j + 1]
				],
				vertexNormals[
					interpolationVertexIndices[j]
				],
				normalDelta
			);
	...
	function toon(color, interpolationWeight) {
		...
		var rounding = (sumOfAllLightTypesIntensity / 4);
		vec3.scale(color.rgba, Math.round(intensity['total'] * rounding) / rounding, color.rgbaShaded);
		//vec3.scale(color.rgba, intensity['total'], color.rgbaShaded);
	}
