...
define(["exports", "data", "glMatrix"], function(data, exports) {
	...
	function calcuateVertexNormalsFromPolygonNormals(){
		...
		for (let i = 0; i < this.polygonVertices.length; i++) {
			let polygon = this.polygonVertices[i];
			let normal = this.polygonNormals[i];

			// Loop over vertices of polygon.
			for (let j = 0; j < polygon.length; j++) {
				// Accumulate/add all polygon normals.
				this.vertexNormals[polygon[j]][0] += normal[0];
				this.vertexNormals[polygon[j]][1] += normal[1];
				this.vertexNormals[polygon[j]][2] += normal[2];
			}
		}

		// Normalize normals.
		this.vertexNormals.forEach(function (n) {
