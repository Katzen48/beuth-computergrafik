...
function(exports, scenegraph, animation) {
	...
	function init() {
		...
		var sjoint = scenegraph.createNodeWithModel("sjoint", "cube", {scale:70})
		sjoint.translate([0,400,0]);

		var shoulder = scenegraph.createNodeWithModel("shoulder", "rect", {scale:60}, sjoint);

		shoulder.translate([0,-200,0]);

		var ejoint = scenegraph.createNodeWithModel("ejoint", "cube", {scale:60},shoulder)
		shoulder.translate([0,-100,0]);
		ejoint.rotate([0,0,0.5]);

		var elbow = scenegraph.createNodeWithModel("elbow", "rect", {scale:40},ejoint);
		elbow.translate([0,-200,0]);

		var hjoint = scenegraph.createNodeWithModel("hjoint", "cube", {scale:30},elbow)
		hjoint.translate([0,-50,0]);
		hjoint.rotate([0,0,0.5]);

		var hand = scenegraph.createNodeWithModel("hand", "rect", {scale:20}, hjoint);
		hand.translate([0,-100,0]);
		hand.rotate([0,1.5,0]);
