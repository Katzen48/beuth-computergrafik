...
function(exports, scenegraph, animation) {
	...
	function init() {
		...
		var solarSystem = scenegraph.createNodeWithModel("solarSystem", "empty", {scale:100, textureURL:"brickWall.jpg"});
		var earthOrbit = scenegraph.createNodeWithModel("earthOrbit", "empty", {scale:100, textureURL:"brickWall.jpg"});
		var moonOrbit = scenegraph.createNodeWithModel("moonOrbit", "empty", {scale:100, textureURL:"brickWall.jpg"});

		var sun = scenegraph.createNodeWithModel("sun", "cube", {scale:100, textureURL:"brickWall.jpg"});
		var earth = scenegraph.createNodeWithModel("earth", "cube", {scale:50, textureURL:"land_ocean_ice_2048.jpg"});
		earth.translate([-100,-300,0]);
		earth.rotate([1.5,2.5,-1.5]);
		var moon = scenegraph.createNodeWithModel("moon", "cube", {scale:10, textureURL:"uvTest.jpg"});
		moon.translate([-100,-100,0]);
		moon.rotate([-1,-1,-1]);

		sun.setParent(solarSystem);
		earthOrbit.setParent(solarSystem);
		earth.setParent(earthOrbit);
		moonOrbit.setParent(earth);
		moon.setParent(moonOrbit);

		animation.assign(moonOrbit, "rotate", {rotationSpeed: [0.04, 0, 0]});
		animation.assign(earthOrbit, "rotate", {rotationSpeed: [0, 0, 0.01]});
